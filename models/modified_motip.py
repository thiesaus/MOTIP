...
from .deformable_detr_plus.deformable_detr import build as build_deformable_detr
from .dab_deformable_detr_plus.dab_deformable_detr import build_dab_deformable_detr
...
class MOTIP(nn.Module):
    ...
    def forward(self, frames, 
                detr_checkpoint_frames: int | None = None, 
                targets=None, max_pad=None,
                sentences=None):
        if detr_checkpoint_frames is not None:
            ...
            if targets is None:
                detr_outputs = self.detr(frames, sentences=sentences)
            else:
                detr_outputs = self.detr(frames, targets, max_pad, sentences=sentences)

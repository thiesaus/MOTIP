...
from .deformable_transformer import build_deformable_transformer, FeatureResizer, VisionLanguageFusionModule
import copy
...
class DeformableDETR(nn.Module):
    ...
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels,
        ...
        # language encoder
        self.tokenizer = RobertaTokenizerFast.from_pretrained('./pretrains/roberta-base/', local_files_only=True)
        self.text_encoder = RobertaModel.from_pretrained('./pretrains/roberta-base/', local_files_only=True)
        # prepare huggingface pretrained files
        # >>> python
        # >>> from transformers import RobertaModel, RobertaTokenizerFast
        # >>> tokenizer = RobertaTokenizerFast.from_pretrained('roberta-base')
        # >>> encoder = RobertaModel.from_pretrained('roberta-base')
        # >>> tokenizer.save_pretrained("./pretrains/roberta-base/")
        # >>> encoder.save_pretrained("./pretrains/roberta-base/")
    ...
    def forward(self, samples: NestedTensor, sentences):
        ...
        text_word_features, text_word_mask, text_sentence_features = self.forward_text(sentences, samples.tensors.device)
...
class SetCriterion(nn.Module):
    ...
    def loss_refers(self, outputs, targets, indices, num_boxes):
        filtered_idx = []
...
def build(args):
    ...
    # add prompt
    weight_dict['loss_refer'] = args.refer_loss_coef
    if args.masks:
    ...
    losses = ['labels', 'boxes', 'cardinality', 'refer']
    if args.masks:
        losses += ["masks"]

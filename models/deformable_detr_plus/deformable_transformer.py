...
class VisionLanguageFusionModule(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        assert key.shape == value.shape, "bruh"
        if query.shape[1] != key.shape[1]:
            key = key.repeat(1, query.shape[1], 1)
            value = value.repeat(1, query.shape[1], 1)
            memory_key_padding_mask = memory_key_padding_mask.repeat(query.shape[1], 1)

        tgt2 = self.multihead_attn(query=query, key=key, value=value, 

...
from models.modified_motip import build as build_model
from models.modified_motip import MOTIP
...
def train(config: dict, logger: Logger):
    ...
    if config["DETR_PRETRAIN"] is not None:
        load_detr_pretrain(model=model, pretrain_path=config["DETR_PRETRAIN"], num_classes=config["NUM_CLASSES"], is_motip_pretrain=True)
        logger.print(f"Load MOTIP-DETR pretrain model from {config['DETR_PRETRAIN']}.")
    else:
...
def train_one_epoch(config: dict, model: MOTIP, logger: Logger,
    ...
    for i, batch in enumerate(dataloader):
        ...
        # add prompt
        metrics["refer_loss"].update(detr_loss_dict["loss_refer"].item())
